...
import { computePayoutFromSellingPrice } from "@/lib/config";
...
async function processSingleListing(
  ...
  const payout = computePayoutFromSellingPrice(salePrice);
  ...
  for (let i = 0; i < qty; i++) {
    const row = listingsTable.insert({
      ...
      payout,  // Calculated from salePrice
      sale_price: salePrice,  // Consignor's entered selling price
...
export async function POST(req: NextRequest) {
  ...
  if (Array.isArray(body.listings)) {
    const items: { variantId: string; salePrice: number; quantity?: number }[] = body.listings;
